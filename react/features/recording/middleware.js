...
import { createRecordingEvent, sendAnalytics } from "../analytics";
...
MiddlewareRegistry.register(({ dispatch, getState }) => (next) => (action) => {
    ...
    switch (action.type) {
        case APP_WILL_MOUNT:
            ...
            dispatch(
                registerSound(RECORDING_OFF_SOUND_ID, RECORDING_OFF_SOUND_FILE)
            );

            dispatch(
                registerSound(RECORDING_ON_SOUND_ID, RECORDING_ON_SOUND_FILE)
            );
        ...
        case RECORDING_SESSION_UPDATED: {
            ...
            const updatedSessionData = getSessionById(
                getState(),
            ...
            if (
                updatedSessionData.status === PENDING &&
                ...
                if (
                    ...
                    typeof APP === 'object' && APP.API.notifyRecordingStarted(mode);
                    ...
                    typeof APP === 'object' && APP.API.notifyRecordingStopped(mode);
...
function _showRecordingErrorNotification(recorderSession, dispatch) {
    ...
    APP.API.notifyNotificationMessage({ "message": "Recording Error" });
